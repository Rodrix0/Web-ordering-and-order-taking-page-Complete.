    ...
    function performSearch() {
        ...
        async function loadMenuItems() {
            try {
                ...
                const response = await fetch('/frontend/buyer/scripts/menu/menuItems.js');
                if (!response.ok) {
    ...
    function displaySearchResults(results) {
        ...
        if (results.length === 0) {
            ...
            results.slice(0, 8).forEach(item => { // Limitar a 8 resultados para mejor rendimiento
                ...
                let htmlContent = `
                    <img src="${item.image}" alt="${item.name}" onerror="this.src='/frontend/buyer/assets/images/default-food.jpg'">
